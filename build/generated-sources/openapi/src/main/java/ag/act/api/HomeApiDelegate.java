...
public interface HomeApiDelegate {
     ...
     * @return Success (status code 200)
     * @see HomeApi#getHomeLink
     */
    default ResponseEntity<HomeLinkResponse> getHomeLink(String linkType) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
     ...
     * @return Success (status code 200)
     * @see HomeApi#getMySolidarityList
     */
    default ResponseEntity<MySolidarityDataArrayResponse> getMySolidarityList(Integer page,
        Integer size,
        List<String> sorts) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
     ...
     * @see HomeApi#updateMySolidarityListDisplayOrder
     */
    default ResponseEntity<MySolidarityDataArrayResponse> updateMySolidarityListDisplayOrder(UpdateMySolidarityRequest updateMySolidarityRequest) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString = "{ \"data\" : [ null, null ], \"paging\" : { \"size\" : 10, \"totalPages\" : 10, \"page\" : 1, \"sorts\" : \"createdAt:desc,userId:asc\", \"totalElements\" : 10 } }";
