...
public class CreateBoardGroupPostReplyCommentCreationCheckAopApiIntegrationTest extends AbstractCommonIntegrationTest {
    ...
    class WhenUserHasCommentedWithinLimitedTime {
        private LocalDateTime replyCommentCreationTime;

        @BeforeEach
        void setUp() {
            replyCommentCreationTime = now.minusSeconds(someIntegerBetween(0, LIMIT_SECONDS - 1));
        }
        ...
        class AndUserIsNotAdmin {
            ...
            void setUp() {
                createNotAdminUser();
                createReplyCommentAtTime(replyCommentCreationTime);

                remainingSeconds = Duration.between(now, replyCommentCreationTime.plusSeconds(LIMIT_SECONDS)).getSeconds();
            ...
            class AndCommentingCooldownIsExpired {
                @BeforeEach
                void setUp() {
                    given(postAndCommentAopCurrentDateTimeProvider.getSecondsUntil(replyCommentCreationTime.plusSeconds(LIMIT_SECONDS)))
                        .willReturn(0L);
                }

                @Test
                @DisplayName("답댓글을 작성한다.")
    ...
    class WhenUserHasNotCommentedWithinLimitedTime {
        ...
        class AndUserHasCommentedMoreThanLimitedTimeAgo {
            ...
            @BeforeEach
            void setUp() {
                LocalDateTime replyCommentCreationTime = now.minusSeconds(someIntegerBetween(1, LIMIT_SECONDS) + LIMIT_SECONDS);
                createReplyCommentAtTime(replyCommentCreationTime);
            }
    ...
    private void createComment() {
        comment = itUtil.createComment(post.getId(), author, CommentType.POST, Status.ACTIVE);
    }

    private void createReplyCommentAtTime(LocalDateTime createdAt) {
        Comment reply = itUtil.createComment(post.getId(), user.getId(), comment.getId(), CommentType.REPLY_COMMENT, Status.ACTIVE);
        reply.setCreatedAt(createdAt);
        itUtil.updateComment(reply);
    }
    ...
    private CommentDataResponse callApiAndGetResult(ResultMatcher resultMatcher) throws Exception {
        return objectMapperUtil.toResponse(callApi(resultMatcher).getResponse().getContentAsString(), ag.act.model.CommentDataResponse.class);
    }

    private MvcResult callApi(ResultMatcher resultMatcher) throws Exception {
        return mockMvc
            .perform(
                post(TARGET_API, stock.getCode(), board.getGroup().name(), post.getId(), comment.getId())
