...
class PostsDuplicateWithNotificationApiIntegrationTest extends AbstractCommonIntegrationTest {
    ...
    private Long anotherStockGroupId;
    ...
    void setUp() {
        ...
        stock = createStock(user, targetStockCode);
        ...
        anotherStockGroupId = anotherStockGroup.getId();
        itUtil.createStockGroupMapping(anotherStockCode, anotherStockGroup.getId());

        // 복제할 갯수, 테스트를 위한 가짜 갯수 생성
        int targetStockGroupAndMappingCount = someIntegerBetween(2, 10);
        int anotherStockGroupAndMappingCount = someIntegerBetween(2, 10);
        ...
        // 다른 종목그룹에 속한 게시판 생성
        for (int i = 0; i < anotherStockGroupAndMappingCount; i++) {
            anotherStockGroupBoardIds.add(createStockAndBoardInSameStockGroup(anotherStockGroup));
        }
    ...
    private void assertValidBoardPostsFromDatabase(List<Long> boardIds) {
        for (Long boardId : boardIds) {
            ...
            assertThat(duplicatedPost.getIsNotification(), is(post.getIsNotification()));
            assertThat(duplicatedPost.getIsExclusiveToHolders(), is(post.getIsExclusiveToHolders()));
            ...
            final Stock newStock = itUtil.findStock(duplicatedPost.getBoard().getStockCode());
            assertThat(newStock.getCode(), not(stock.getCode()));

            if (isEtcDigitalDocument(duplicatedPost)) {
                assertThat(duplicatedPost.getDigitalDocument().getCompanyName(), is(newStock.getName()));
            }

            final Optional<Notification> optionalNotification = itUtil.findNotificationByPostId(duplicatedPost.getId());
    ...
    private boolean isEtcDigitalDocument(Post duplicatedPost) {
        return duplicatedPost.getDigitalDocument() != null
            && duplicatedPost.getDigitalDocument().getType() == DigitalDocumentType.ETC_DOCUMENT;
    }
    ...
    @Nested
    class WhenDuplicatePostInTheSameStockGroupSuccess {
        ...
        @Nested
        class AndEtcDigitalDocument {

            @BeforeEach
            void setUp() {
                final User acceptUser = itUtil.createUser();
                itUtil.createDigitalDocument(post, stock, acceptUser, DigitalDocumentType.ETC_DOCUMENT);
            }
    ...
    @Nested
    class WhenDuplicatePostToTheAnotherStockGroupSuccess {

        @DisplayName("Should return 200 response code when call " + TARGET_API)
        @Test
        void shouldReturnSuccessMessage() throws Exception {
            final ag.act.model.SimpleStringResponse result = callApi(anotherStockGroupId);

            assertThat(result.getStatus(), containsString("초 동안 %s건의 게시글을 복제하였습니다.".formatted(anotherStockGroupBoardIds.size())));
            assertValidBoardPostsFromDatabase(anotherStockGroupBoardIds);
            assertEmptyStockGroupBoardPostsFromDatabase(targetBoardIds);
        }
    }
}
