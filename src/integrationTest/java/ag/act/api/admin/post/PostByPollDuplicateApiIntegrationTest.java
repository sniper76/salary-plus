...
class PostByPollDuplicateApiIntegrationTest extends AbstractCommonIntegrationTest {
    ...
    class WhenDuplicatePostSuccess {
        ...
        class AndPollPost {
            ...
            private void assertResponse(PostDetailsDataResponse result) {
                ...
                final PollResponse afterPoll = createResponse.getPoll();
                ...
                assertThat(afterPoll.getSelectionOption(), is(beforePoll.getSelectionOption().name()));
                assertThat(afterPoll.getVoteType(), is(beforePoll.getVoteType().name()));
                ...
                final List<PollItemResponse> afterPollItemList = afterPoll.getPollItems();
                ...
                for (int k = 0; k < afterPollItemList.size(); k++) {
                    PollItemResponse afterPollItem = afterPollItemList.get(k);
        ...
        class AndPollsPost {
            ...
            void setUp() {
                ...
                final LocalDateTime startDate = now.minusDays(1);
                final LocalDateTime endDate = now.plusDays(3);
                itUtil.createPolls(post, 2, 2, SelectionOption.SINGLE_ITEM, startDate, endDate);
                postId = post.getId();
            }
            ...
            private void assertResponse(PostDetailsDataResponse result) {
                ...
                final List<PollResponse> afterPolls = createResponse.getPolls();
                final List<Poll> beforePolls = post.getPolls();
                for (int index = 0; index < afterPolls.size(); index++) {
                    PollResponse afterPoll = afterPolls.get(index);
                    final Poll beforePoll = beforePolls.get(index);
                    assertThat(afterPoll, is(notNullValue()));
                    assertThat(afterPoll.getTitle(), is(beforePoll.getTitle()));
                    assertThat(afterPoll.getSelectionOption(), is(beforePoll.getSelectionOption().name()));
                    assertThat(afterPoll.getVoteType(), is(beforePoll.getVoteType().name()));
