...
class CreateCommentReplyApiIntegrationTest extends AbstractCommonIntegrationTest {

    private static final String TARGET_API = "/api/admin/stocks/{stock}/board-groups/{boardGroup}/posts/{postId}/comments/{commentId}/replies";
    ...
    void setUp() {
        ...
        comment = itUtil.createComment(post.getId(), user1, CommentType.POST, Status.ACTIVE);
        final User user2 = itUtil.createUser();
        itUtil.createComment(post.getId(), user2.getId(), comment.getId(), CommentType.REPLY_COMMENT, Status.ACTIVE);
    }
    ...
    @Nested
    class WhenIsAdminWriteMoreReplies {

        @DisplayName("Should when different admin users make replies")
        @Test
