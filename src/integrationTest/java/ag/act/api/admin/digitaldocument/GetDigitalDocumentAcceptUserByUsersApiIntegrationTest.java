...
class GetDigitalDocumentAcceptUserByUsersApiIntegrationTest extends AbstractCommonIntegrationTest {

    private static final String TARGET_API = "/api/admin/digital-document/{digitalDocumentId}/users";

    private String acceptUserJwt;
    ...
    void setUp() {
        itUtil.init();
        final User adminAcceptUser = itUtil.createAcceptorUser();
        acceptUserJwt = itUtil.createJwt(adminAcceptUser.getId());
        ...
        itUtil.createSolidarityLeader(solidarity, adminAcceptUser.getId());

        final DigitalDocument digitalDocument = createDigitalDocument(adminAcceptUser);
    ...
    private DigitalDocument createDigitalDocument(User acceptUser) {
    ...
    class WhenError {
        ...
        void setUp() {
            itUtil.init();
            final User adminAcceptUser = itUtil.createAcceptorUser();
            final User otherAcceptUser = itUtil.createAcceptorUser();
            acceptUserJwt = itUtil.createJwt(adminAcceptUser.getId());
            ...
            final Solidarity solidarity = itUtil.createSolidarity(stock.getCode());
            itUtil.createSolidarityLeader(solidarity, otherAcceptUser.getId());

            final DigitalDocument digitalDocument = createDigitalDocument(otherAcceptUser);
            digitalDocumentId = digitalDocument.getId();
            itUtil.createDigitalDocumentItemList(digitalDocument);
        ...
        void shouldReturnException() throws Exception {
            ...
            itUtil.assertErrorResponse(response, 404, containsString("전자문서 정보가 없습니다. 수임인명"));
        }
    }
