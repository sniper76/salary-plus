...
import static ag.act.TestUtil.someDigitalDocumentTypeExceptEtcDocument;
...
class CreateAllUsersSignatureOpportunityOfDigitalDocumentsExceptEtcCountCsvBatchIntegrationTest
    ...
    private void commonScenario() {
        ...
        createDigitalDocument(
            someDigitalDocumentTypeExceptEtcDocument(), user, digitalDocumentStartDate, digitalDocumentEndDate, post1, stock1
        );

        createDigitalDocument(
            DigitalDocumentType.ETC_DOCUMENT, user, digitalDocumentStartDate, digitalDocumentEndDate, post2, stock2
        );
    ...
    private String getExpectedFileFullPath() {
        return String.format(
            ...
            FilenameUtil.getFilenameWithDate(
                ALL_USERS_SIGNATURE_OPPORTUNITY_GIVEN_HAVE_STOCK_IN_DIGITAL_DOCUMENTS_EXCEPT_ETC_PROGRESS_PERIOD.getFileName(),
                "csv",
                referenceDate
            )
        );
    ...
    private List<String> scenarioForUser1() {
        ...
        // week3 : 가지고 있는 종목 2개에 대한 전자문서(기타문서 제외) 서명 기회 1회
        itUtil.createUserHoldingStockHistoryOnDate(
            user1.getId(), stock1.getCode(), generateRandomDateSameOrAfterReferenceDateWithinWeek(week3, digitalDocumentStartDate));
        itUtil.createUserHoldingStockHistoryOnDate( // 기타문서 기간에 종목 보유
        ...
        expectedRow1Cells.add("1"); // user1 holding stock1 history during digital documents excepted Etc document progress period at week3

        // week4 : 가지고 있는 종목 2개에 대한 전자문서(기타문서 제외) 서명 기회 1회
        itUtil.createUserHoldingStockHistoryOnDate(
            user1.getId(), stock1.getCode(), generateRandomDateBeforeReferenceDateWithinWeek(week4, digitalDocumentEndDate));
        itUtil.createUserHoldingStockHistoryOnDate( // stock2 를 가지고 있는 기간에 해당 종목의 전자문서가 열리지 않음 & 기타문서 기간에 종목 보유
        ...
        expectedRow1Cells.add("1"); // user1 holding stock1 history during digital document excepted Etc document progress period at week4
    ...
    private List<String> scenarioForUser2() {
        ...
        // week3 : 가지고 있는 종목 2개에 대한 전자문서 서명 기회 0회
        ...
        expectedRow2Cells.add("0"); // user2 at week3
        ...
        expectedRow2Cells.add("1"); // user2 holding stock2 history during digital documents excepted Etc document progress period at week4
    ...
    private MvcResult callApi() throws Exception {
        return mockMvc
            .perform(
                post(
                    TARGET_API,
                    getPathFrom(ALL_USERS_SIGNATURE_OPPORTUNITY_GIVEN_HAVE_STOCK_IN_DIGITAL_DOCUMENTS_EXCEPT_ETC_PROGRESS_PERIOD.name())
                )
    ...
    private void assertResponse(MvcResult response) throws Exception {
        ...
        final String batchName = ALL_USERS_SIGNATURE_OPPORTUNITY_GIVEN_HAVE_STOCK_IN_DIGITAL_DOCUMENTS_EXCEPT_ETC_PROGRESS_PERIOD.name();
