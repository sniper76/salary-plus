...
@Transactional
public class UpdateSolidarityDailySummariesBatchExecutor implements IBatchExecutor {
    ...
    public boolean supports(BatchParameter batchParameter) {
        return BatchName.UPDATE_SOLIDARITY_DAILY_SUMMARIES.equals(batchParameter.getBatchName());
    }
            ...
            (Consumer<Integer> batchCountLog) -> {
                ...
                solidarities.forEach(solidarity -> {
                    ...
                    solidarity.setMostRecentDailySummary(
                        solidarityDailySummaryService.updateSolidarityDailySummary(solidarity)
                    );
                ...
                return "[Batch] %s batch successfully finished. [modification: %s / %s]".formatted(batchName, count.get(), totalCount);
            }
        );
    }
}
