...
class UpdateSolidarityDailySummariesBatchExecutorTest {

    @InjectMocks
    private UpdateSolidarityDailySummariesBatchExecutor batch;
    ...
    @Nested
    class UpdateSolidarityDailySummaries {
        @Mock
        ...
        private void mockSolidarity(Solidarity solidarity, SolidarityDailySummary summary, SolidarityDailySummary existingSummary) {
            given(solidarityDailySummaryService.updateSolidarityDailySummary(solidarity))
                .willReturn(summary);
        ...
        void shouldReturnResultMessage() {
            assertThat(actualMessage, Matchers.is(
                "[Batch] %s batch successfully finished. [modification: %s / %s]"
                    .formatted(batchName, 2, 3)
            ));
        }
        ...
        void shouldSetMostRecentDailySummary() {
            ...
            then(solidarity3).should(never()).setMostRecentDailySummary(solidarityDailySummary3);
        }
        ...
        void shouldCallSaveSolidarity() {
            ...
            then(solidarityService).should(never()).saveSolidarity(solidarity3);
        }
    }
}
